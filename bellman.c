#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <limits.h>

int INFINITE = INT_MAX;

struct Edge {
  int src, dest, weight;
};

struct Graph {
  int V, E;
  struct Edge* edge;
};

struct Graph* createGraph(int V, int E) {
  struct Graph* graph;

  graph-> V = V;
  graph-> E = E;

  struct Edge* edges[E];
  graph->edge = edges;

  return graph;
}

void printResults(int dist[], int n) {
  printf("Vertex   Distance from Source\n");

  for (int i = 0; i < n; ++i) {
    if(dist[i] == INT_MAX)
      printf("%d \t\t %s\n", i, "INFINITE");
    else
      printf("%d \t\t %d\n", i, dist[i]);
}
}

void RelaxEdges(struct Graph *graph, int *dist, int V, int E) {
  for (int i = 1; i <= V-1; i++) {
    for (int j = 0; j < E; j++) {
      int u = graph->edge[j].src;
      int v = graph->edge[j].dest;
      int weight = graph->edge[j].weight;
      if (dist[u] != INFINITE && dist[u] + weight < dist[v])
          dist[v] = dist[u] + weight;
    }
  }
}

void CheckNegativeWeightCycle(struct Graph *graph, int *dist, int E) {
  for (int i = 0; i < E; i++) {
    int u = graph->edge[i].src;
    int v = graph->edge[i].dest;
    int weight = graph->edge[i].weight;
    if (dist[u] != INFINITE && dist[u] + weight < dist[v])
      printf("Graph contains negative weight cycle");
  }
}

void InitializeGraph(int *dist, int V, int src) {
    for (int i = 0; i < V; i++)
      dist[i]   = INFINITE;
      dist[src] = 0;
}

void BellmanFord(struct Graph* graph, int src) {
  int V = graph->V;
  int E = graph->E;
  int dist[graph->V];

  InitializeGraph(dist, V, src);
  RelaxEdges(graph, dist, V, E);
  CheckNegativeWeightCycle(graph, dist, E);
  printResults(dist, V);
}

int main() {
  /* Let us create the graph given in above example */
  int V = 10;  // Number of vertices in graph
  int E = 18;  // Number of edges in graph

  printf("Enter graph values: \n");
  scanf("%d",&V);
  scanf("%d",&E);

  struct Graph* graph = createGraph(V, E);

  int i;
  for(i=0;i<E;i++) {
    scanf("%d",&graph->edge[i].src);
    scanf("%d",&graph->edge[i].dest);
    scanf("%d",&graph->edge[i].weight);
  }

  BellmanFord(graph, 0);

  return 0;
}
